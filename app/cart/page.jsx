...
export default function CartPage() {
  ...
  function handleQtyMinus(item) {
    if (item.qty > 1) {
      ...
      const confirmRemove = window.confirm(t.cart.confirmRemove || "Remove this item?");
      if (confirmRemove) {
  ...
  function handleRemoveItem(id) {
    const confirmRemove = window.confirm(t.cart.confirmRemove || "Are you sure?");
    if (confirmRemove) {
      removeFromCart(id);
    }
  }
  ...
  if (orderDone) {
    return (
      <div className="container" style={{ paddingTop: 70, paddingBottom: 70, textAlign: "center" }}>
        <h1 style={{ fontSize: 26, marginBottom: 14 }}>✓ {t.cart.orderSuccess}</h1>
        ...
        <div className={styles.layout}>
            ...
            {items.map((item) => (
              <div key={item.id} className={styles.row}>
                <div className={styles.rowImage}>
                  <Image src={item.image} alt={item.name[lang] || ""} fill style={{ objectFit: "cover" }} />
                </div>
                ...
                <div className={styles.qtyControl}>
                  <button type="button" onClick={() => handleQtyMinus(item)} aria-label="-">
                    −
                  </button>
                  <span>{item.qty}</span>
          ...
          <div className={styles.summary}>
            ...
            <div className={styles.summaryRow}>
              ...
              <span>
                {delivery === 0 ? t.cart.free : `${delivery} ${t.common.currency}`}
              </span>
            </div>
            ...
            {!checkoutOpen && (
              <button className="btn btn-block" onClick={() => setCheckoutOpen(true)}>
                {t.cart.checkout}
              </button>
            )}
            ...
            {checkoutOpen && (
              <form className={styles.checkoutForm} onSubmit={handlePlaceOrder}>
                <label className={styles.formTitle}>{t.cart.payment}</label>
                <div className={styles.paymentOptions}>
                  <button
                    type="button"
                    className={payment === "cod" ? styles.paymentActive : styles.paymentBtn}
                  ...
                  <button
                    type="button"
                    className={payment === "visa" ? styles.paymentActive : styles.paymentBtn}
                ...
                <div className={styles.field}>
                  ...
                  <textarea
                    ...
                    name="notes"
                    rows={3}
                    placeholder={t.cart.notesPlaceholder || "Write any notes about your order..."}
                    value={form.notes}
                    onChange={handleChange}
                  />
                </div>
                ...
                <button type="submit" className="btn btn-block" style={{ marginTop: 10 }}>
                  {t.cart.placeOrder}
                </button>
              </form>
            )}
